...
def plot_from_func(funcs: list[Callable[[float], float]], 
                    ...
                    right_end: int, 
                    num_of_points: int, 
                    knots: list[tuple[list[float], list[float]]],
                    title: str) -> None:
    ...
    plt.xlabel("x")
    plt.ylabel("f(x)")
    ax.plot(knots[0][0], knots[0][1], linestyle="None", marker="o", markerfacecolor="black", label="Chebyshev points")
    ax.plot(knots[1][0], knots[1][1], linestyle="None", marker="o", markerfacecolor="orange", label="Equal points")
